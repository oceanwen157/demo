...
(function (doc, win) {
    ...
    Launcher({
        ...
        //过滤条件
        let originList = Array.prototype.slice.call( $(".filter-wrap .filter-setting"))
        $("#filterInput").on('input', function() {
            let value = $(this).val();
            let res = originList.filter(item=>{
                return  $(item).attr("data-value").indexOf(value) !== -1
            })
            $(".filter-wrap ").html(res);
            //todo
        ...
        $("#submit-btn").on('click',function(){
            let res = []
            Array.prototype.slice.call($(".content-filter-widget input:checked")).map(item=>{
                res.push($(item).val())
            })
            // // 添加日期 
            // Duration 
        ...
        $("#getmore").on("click",function(){
            ...
            $(".filter-wrap ").append(content);
            originList = Array.prototype.slice.call( $(".filter-wrap .filter-setting"))
        })
